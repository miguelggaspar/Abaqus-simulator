...
#Parameters fetched by the server
length = int(sys.argv[-4].strip())          # strip will chop the '\n'
youngsModulus = int(sys.argv[-3].strip())
poissonsRatio = int(sys.argv[-2].strip())
jobName = sys.argv[-1]

#Check if any of given parameters are empty
if length == -1:
    length = 100;

if youngsModulus == -1:
    youngsModulus = 209.E3
else:
    youngsModulus=youngsModulus*(10**3)     #GPa to MPa

if poissonsRatio == -1:
    poissonsRatio = 0.3
else:
    poissonsRatio = poissonsRatio * (10**-1)
# Create a model.
...
mySketch.rectangle(point1=(-length,10), point2=(length,-10))
